...
export const getMyInsights = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [mentions1, mentions7, news7, lastSnap] = await Promise.all([
      supabase
    ...
    // Trend: compare last 24h negative share vs prior 6 days average
    ...
    if (lovableKey && (n7.length || m1.length)) {
      try {
        ...
        const sampleNews = n7
          ...
          .join("\n");
        const prompt = `Você é um estrategista de comunicação política. Com base nos dados abaixo, devolva exatamente 3 recomendações práticas de resposta para as próximas 48h, no formato JSON: {"recs":["...","...","..."]}. Frases curtas (máx 180 caracteres), em português, tom institucional.

Tendência de sentimento (últimos 7 dias): ${trend}.
